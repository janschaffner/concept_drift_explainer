"""
This module contains the implementation of the Explanation Agent.

This is the final agent in the CDE pipeline. Its primary
responsibility is to synthesize all the curated and classified evidence into a
coherent, cautious, and human-readable explanation for the detected concept
drift. It employs a sophisticated, multi-step process that includes context
expansion, drift-type-specific cause generation, a self-correction/refinement
step, and final summary generation to produce a high-quality, trustworthy output.
"""
...
def format_context_for_prompt(classified_context: list) -> str:
    """Formats a list of context snippets into a markdown string for the LLM prompt.

    Args:
        classified_context: A list of ContextSnippet objects.

    Returns:
        A formatted string presenting the evidence clearly to the LLM.
    """
    formatted_str = ""
...
def expand_context(snippets: List[Dict], index: Pinecone.Index) -> List[Dict]:
    ...
    This function is critical for resolving hallucination bugs. It takes the
    curated list of top-ranked snippets, identifies their unique source documents,
    and then queries Pinecone with a metadata filter to retrieve the *full text*
    of each document. This provides the LLM with complete context for its
    analysis, rather than just an isolated, potentially out-of-context chunk.

    Args:
        snippets: The curated list of snippets from the Re-Ranker Agent.
        index: The initialized Pinecone index connection.

    Returns:
        A new list of snippet-like objects, where each object represents a
        full document's merged text, sorted by the original priority score.
    """
...
def calculate_confidence_score(snippet: Dict, drift_info: Dict, rank: int) -> float:
    """Calculates a data-driven confidence score for a potential cause.

    This score is a transparent, programmatic calculation based on concrete
    data signals from the pipeline. It is not generated by the LLM.

    The formula is:
    - Start with the `priority_score` from the Re-Ranker as the baseline.
    - Add a `temporal_bonus` that rewards documents published closer to the
      drift's start date.
    - Apply a `rank_multiplier` to give a significant boost to the top-ranked
      causes from the Re-Ranker.

    Args:
        snippet: The evidence snippet object for this cause.
        drift_info: The structured drift information.
        rank: The 0-based final rank of this cause.

    Returns:
        The final, calibrated confidence score as a float.
    """
    ...
    # Apply a multiplier based on the final rank determined by the Re-Ranker.
...
def run_explanation_agent(state: GraphState, index: Pinecone.Index) -> dict:
    """The final synthesizing agent in the CDE pipeline.

    This agent orchestrates a complex, multi-step process to generate the final
    explanation:
    1.  **Context Expansion**: Expands the top-ranked snippets into full documents.
    2.  **Individual Cause Generation**: Iterates through each full document and
        uses a drift-type-specific prompt to generate a single, focused cause.
    3.  **Self-Correction/Refinement**: Passes all generated causes to a powerful
        "editor" LLM for a final review and refinement step.
    4.  **Summary Generation**: Creates a final executive summary based on the
        refined causes.
    ...
    Returns:
        A dictionary with the final `explanation` object.
    """
    logging.info("--- Running Explanation Agent ---")
    
    # Step 0: Initial setup and data extraction from the state.
    ...
    # Step 1: Expand the context of the top snippets to their full documents.
    # This is a critical step to prevent the LLM from hallucinating.
    ...
    # Step 2: Loop through each expanded document and generate a "draft" cause
    # individually. This one-to-one mapping is key to ensuring traceability.
    try:
        ...
        # "Normal" case (more than one document)
        # Loop through each evidence doc and generate a cause
        draft_causes = []
        ...
        # Step 3: Use a "Self-Correction" or "Editor" prompt to refine the
        # collection of draft causes, improving quality and consistency.
        ...
        # Re-attach the original, data-driven confidence scores to the refined
        # causes to ensure the final output remains grounded in data.
        ...
        # Step 4: Generate a final executive summary based on the refined causes.
